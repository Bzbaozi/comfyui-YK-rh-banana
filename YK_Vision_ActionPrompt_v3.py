# YK_Vision_ActionPrompt_v3.py - Banana2 图像编辑专用提示词生成器（角度选择版）
...
class YK_Vision_ActionPrompt_v3:
    # 品类 → 显示名称映射
    GARMENT_DISPLAY_MAP = {
        # 上装
        "上衣": "上衣",
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            ...
            "optional": {
                ...
                "坐姿支撑物": ([
                    "现代简约椅", "北欧风椅", "设计师款椅", "时尚沙发",
                    "布艺凳", "木椅", "高脚凳", "台阶/平台", "无具体支撑"
                ], {"default": "现代简约椅", "tooltip": "电商服装摄影推荐：优先选择具有设计感的支撑物，避免过于日常或杂乱的款式"}),
                "坐姿支撑物描述": ("STRING", {"default": "", "multiline": True, "tooltip": "详细描述支撑物的颜色、样式、材质、造型，确保分批生成时一致性，如：'米白色现代简约椅'、'浅灰色布艺沙发'"}),
                "支撑物一致性模式": ([
                    "标准模式",
                    "精确模式",
                    "严格模式"
                ], {"default": "精确模式", "tooltip": "控制支撑物描述的严格程度：标准=基本描述，精确=详细描述，严格=强制使用相同的支撑物特征"}),
                "提示词语言": ([
                    "中文",
                    "英文"
                ], {"default": "英文", "tooltip": "选择提示词语言，英文通常对生图模型效果更好"}),
            }
        }
    ...
    def _is_full_body_shot(self, pil_img):
        """
        根据图片宽高比判断是否为全身图
        全身图通常宽高比接近1:1或更窄（高度大于宽度）
        半身/特写图通常宽度大于高度或高度远小于宽度
        返回：True=全身图，False=非全身图
        """
        width, height = pil_img.size
        aspect_ratio = width / height
        
        # 全身图特征：高度大于或接近宽度
        # 典型全身图比例：1:1, 3:4, 9:16, 2:3等
        # 半身/特写图比例：通常更宽，如4:3, 16:9等
        
        if aspect_ratio <= 1.2:
            # 高度 >= 宽度，很可能是全身图
            self.console_log(f"📐 图像比例 {width}x{height} ({aspect_ratio:.2f}:1) - 判定为全身图")
            return True
        elif aspect_ratio <= 1.5:
            # 接近正方形，可能是全身图也可能是半身图，保守起见判定为全身图
            self.console_log(f"📐 图像比例 {width}x{height} ({aspect_ratio:.2f}:1) - 接近正方形，判定为全身图")
            return True
        else:
            # 宽度明显大于高度，更可能是半身或特写
            self.console_log(f"📐 图像比例 {width}x{height} ({aspect_ratio:.2f}:1) - 判定为非全身图")
            return False
    ...
    def _generate_action_descriptions(self, pil_img, api_key, use_api, api_model,
        ...
        if use_api and api_key.strip() and OPENAI_AVAILABLE and len(angle_list) > 0:
            try:
                ...
                # 检查是否包含坐姿角度，强制要求支撑物
                has_sitting = any("坐姿" in angle for angle, _ in angle_list)
                if has_sitting:
                    system_prompt += (
                        f"\n【坐姿支撑物要求 - 强制执行】\n"
                        f"所有包含'坐姿'的角度，人物必须坐在{sitting_support}上，严禁腾空坐姿。\n"
                        f"所有坐姿必须使用完全相同的一把{sitting_support}，同一个款式、同一种颜色、同一种材质、同一种风格，严禁出现不同样式或不同颜色的{sitting_support}。\n"
                        f"动作描述中必须明确体现坐在{sitting_support}上的支撑关系，例如：'坐在{sitting_support}上，双手放膝盖'、'侧坐在{sitting_support}边缘，单手扶{sitting_support}'。\n"
                        f"禁止出现没有{sitting_support}的悬空坐姿，人物臀部必须有明确的{sitting_support}接触面。\n"
                    )
    ...
    def _build_simple_prompt(self, garment_type, action_desc, is_sitting=False, sitting_support="现代简约椅", sitting_support_desc="", consistency_mode="精确模式", custom_instruction="", prompt_language="英文", is_full_body=False):
        """
        构建固定简洁格式的编辑提示词
        is_sitting: 是否为坐姿角度，若为True则追加支撑物约束
        sitting_support: 坐姿支撑物名称
        sitting_support_desc: 支撑物详细描述（颜色、样式、材质等）
        consistency_mode: 一致性模式（标准模式/精确模式/严格模式）
        custom_instruction: 用户自定义全局指令
        prompt_language: 提示词语言（中文/英文）
        is_full_body: 是否为全身图，若是则不添加脸部遮挡提示
        """
        # 下半身服装品类列表
        lower_body_items = ["裙子", "短裙", "长裙", "裤子", "短裤", "长裤", "牛仔裤", "西装裤"]
        is_lower_body = any(kw in garment_type for kw in lower_body_items)
        
        # 只有非全身图的下半身品类才添加脸部遮挡提示
        need_hide_face = is_lower_body and not is_full_body
        
        # 翻译支撑物名称到英文
        support_translation = {
            "现代简约椅": "modern minimalist chair",
            "北欧风椅": "Scandinavian style chair",
            "设计师款椅": "designer chair",
            "时尚沙发": "stylish sofa",
            "布艺凳": "fabric stool",
            "木椅": "wooden chair",
            "高脚凳": "bar stool",
            "台阶/平台": "step/platform",
            "无具体支撑": "no specific support"
        }
        eng_support = support_translation.get(sitting_support, sitting_support)
        
        if prompt_language == "英文":
            # 英文提示词
            prompt = (
                f"Show {garment_type} as main focus, change the pose of the person in the picture, "
                f"keep scene unchanged, keep background unchanged, keep clothing matching unchanged, keep composition unchanged, "
                f"{action_desc}, "
                f"no extra limbs, single person single image, only one person in the frame"
            )
            # 下半身品类（非全身图）添加脸部遮挡提示
            if need_hide_face:
                prompt += ", do not show face, face must be hidden or out of frame, focus on lower body"
            if is_sitting:
                if sitting_support_desc:
                    if consistency_mode == "严格模式":
                        prompt += f", person must sit elegantly on {sitting_support_desc}, the color, style, material, shape of this support must be exactly the same in all images, pixel-level matching, no minor differences, no ugly supports"
                    elif consistency_mode == "精确模式":
                        prompt += f", person must sit elegantly on {sitting_support_desc}, the color, style, material, shape of this support must be consistent in all sitting pose images, no different styles or colors, no ugly supports"
                    else:
                        prompt += f", person must sit elegantly on {sitting_support_desc}, keep support style consistent, no ugly supports"
                else:
                    if consistency_mode == "严格模式":
                        prompt += f", person must sit elegantly on the exact same {eng_support}, same style, same color, same material, same shape, pixel-level matching, no minor differences, no ugly supports"
                    elif consistency_mode == "精确模式":
                        prompt += f", person must sit elegantly on the exact same {eng_support}, same style, same color, same material, no ugly supports, all sitting poses use the same {eng_support}"
                    else:
                        prompt += f", person must sit elegantly on {eng_support}, keep support style consistent, no ugly supports"
            if custom_instruction.strip():
                prompt += f", {custom_instruction.strip()}"
        else:
            # 中文提示词
            prompt = (
            ...
            # 下半身品类（非全身图）添加脸部遮挡提示
            if need_hide_face:
                prompt += "，一定不能露脸，看不到人物的脸部，画面聚焦于下半身"
            if is_sitting:
                if sitting_support_desc:
                    if consistency_mode == "严格模式":
                        prompt += f"，人物必须优雅地坐在{sitting_support_desc}上，该支撑物的颜色、样式、材质、造型必须在所有图片中完全一致，像素级匹配，严禁任何细微差异，严禁出现不美观的支撑物"
                    elif consistency_mode == "精确模式":
                        prompt += f"，人物必须优雅地坐在{sitting_support_desc}上，该支撑物的颜色、样式、材质、造型必须在所有坐姿图中完全一致，禁止使用不同样式或颜色的支撑物，严禁出现不美观的支撑物"
                    else:
                        prompt += f"，人物必须优雅地坐在{sitting_support_desc}上，支撑物样式保持一致，严禁出现不美观的支撑物"
                else:
                    if consistency_mode == "严格模式":
                        prompt += f"，人物必须优雅地坐在完全相同的{sitting_support}上，同一个款式、同一种颜色、同一种材质、同一种造型，像素级匹配，严禁任何细微差异，严禁出现不美观的支撑物"
                    elif consistency_mode == "精确模式":
                        prompt += f"，人物必须优雅地坐在完全相同的{sitting_support}上，同一个款式、同一种颜色、同一种材质，严禁出现不美观的支撑物，所有坐姿使用同一个{sitting_support}"
                    else:
                        prompt += f"，人物必须优雅地坐在{sitting_support}上，支撑物样式保持一致，严禁出现不美观的支撑物"
            if custom_instruction.strip():
                prompt += f"，{custom_instruction.strip()}"
        return prompt
    ...
    def 生成编辑提示词(self, 图像, 使用API, API密钥, API模型, 展示品类, 插口袋动作, 细节数量, 任务总数,
                     ...
                     随机种子, 自定义全局指令="", 坐姿支撑物="现代简约椅", 坐姿支撑物描述="", 支撑物一致性模式="精确模式", 提示词语言="英文"):
        ...
        # === 检测是否为全身图 ===
        is_full_body = self._is_full_body_shot(pil_img)
        full_body_log = f"📏 是否全身图: {'是' if is_full_body else '否'}"
        self.console_log(full_body_log)
        full_log.append(full_body_log)
        ...
        action_descs = self._generate_action_descriptions(
            ...
            seed=随机种子, sitting_support=坐姿支撑物
        )
        ...
        # === 第四步：生成角度提示词 ===
        edit_prompts = []
        for (angle_name, _), desc in zip(angle_list, action_descs):
            is_sitting = "坐姿" in angle_name
            prompt = self._build_simple_prompt(garment_type, desc, is_sitting=is_sitting, sitting_support=坐姿支撑物, sitting_support_desc=坐姿支撑物描述, consistency_mode=支撑物一致性模式, custom_instruction=自定义全局指令, prompt_language=提示词语言, is_full_body=is_full_body)
...
# 注册节点
NODE_CLASS_MAPPINGS = {
    "YK_Vision_ActionPrompt_v3": YK_Vision_ActionPrompt_v3
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "YK_Vision_ActionPrompt_v3": "YK视觉编辑提示词生成器v3（Banana2角度选择版）"
}
